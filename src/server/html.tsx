...
const html = ({
  ...
  const styles = stats.styles.map((filename) => `<link rel="stylesheet" href="${config.app.DIST_URL}/${filename}" />`);
  const scripts = stats.scripts.map(
    (filename) => `<script src="${config.app.DIST_URL}/${filename}" crossorigin></script>`,
  );
  return `
<!DOCTYPE html>
<html lang="en">
